...
| POST   | `/api/v1/auth/signup`     | `{ email, password, displayName }`; creates user/session, returns public user           |
...
Email ownership is not verified in Part 2, and forgotten-password recovery is not implemented. Do not present these accounts as verified identities. The patch implements the specified account/onboarding stage, not all operations needed for a large public service.
